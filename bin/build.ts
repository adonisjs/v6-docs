...
async function generateOgImage(entry: ReturnType<Collection['all']>[0], htmlOutput: string) {
  ...
  /**
   * TEMP: Only generate og images that have description
   */
  if (!description) shouldGenerateOgImage = false
   ...
   */
  const output = `public/og/${entry.title}.png`
   ...
   */
  const ogImageTag = `<meta property="og:image" content="/${output}">`
