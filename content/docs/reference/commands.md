...
The `add` command will install a new package using your package manager and then configure it using the `configure` command. The command accepts the package name as the first argument.
...
Passed down to the `configure` command. Force overwrite files when configuring the package. See `configure` command for more information.
